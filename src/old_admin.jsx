function AdminSettingsViewLive({ computedNav, customNav, setCustomNav, vendorMaster = [], setVendorMaster, categoryMaster = [], setCategoryMaster , companyMaster = [], setCompanyMaster, platformMaster = [], setPlatformMaster}) {
  ...
  const [vendorForm, setVendorForm] = useState({ name: "", category: "" });
  ...
  return (
    <div style={{ background:COLORS.bgCard, borderRadius:20, padding:"24px", border:`1px solid rgba(255,255,255,0.05)`, maxWidth:800, margin:"0 auto" }}>
      {/* Tabs */}
      ...
      {activeTab === "navigation" ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          <div>
            <div style={{ fontSize:18, fontWeight:700, color:COLORS.text }}>Master Data</div>
            <div style={{ fontSize:12, color:COLORS.textMuted }}>View and manage auto-saved Vendors and Categories from the Expense tracker.</div>
          </div>
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <h3 style={{ fontSize:14, color:COLORS.primary, margin: 0 }}>Vendors</h3>
              ...
              {vendorMaster.map((v, i) => (
                <div key={i} style={{ display:"flex", justifyContent:"space-between", background:"rgba(255,255,255,0.03)", border:`1px solid ${COLORS.border}`, borderRadius:8, padding:12 }}>
                  <div>
                    ...
                    <div style={{ fontSize:11, color:COLORS.textMuted }}>{v.category}</div>
                  </div>
                  <div style={{ display: "flex", gap: 10 }}>
