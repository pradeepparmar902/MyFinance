...
# 1. Update routing
old_route = 'case "expense":      return <ExpenseViewLive expenses={expenses} setExpenses={setExpenses} filter={filter} />;'
new_route = 'case "expense":      return <ExpenseViewLive expenses={expenses} setExpenses={setExpenses} filter={filter} subscriptions={subscriptions} setSubscriptions={setSubscriptions} />;'
...
if end_idx == -1: end_idx = code.find("function ReportsViewLive", start_idx)

new_expense_view = """function ExpenseViewLive({ expenses, setExpenses, filter, subscriptions, setSubscriptions }) {
...
if start_idx != -1 and end_idx != -1:
    ...
    with open("src/FinPilotAI.jsx", "w", encoding="utf-8") as f:
        f.write(code)
    print("Successfully patched ExpenseViewLive!")
else:
